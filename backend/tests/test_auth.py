...
def test_load_tokens_and_tier_hierarchy(tmp_path: Path) -> None:
    ...
    assert tokens["rec-abc"]["tier"] == "recruiter"
    assert tokens["pers-xyz"]["tier"] == "personal"

    assert accessible_tiers("public") == ["public"]
    assert accessible_tiers("recruiter") == ["public", "recruiter"]
    assert accessible_tiers("personal") == ["public", "recruiter", "personal"]
...
def test_conversation_and_turn_limits() -> None:
    # public: 3 conversations/day, 15 turns/conversation.
    assert conv_limit("public") == 3
    assert turn_limit("public") == 15
    assert conv_limit("recruiter") == 10
    assert turn_limit("recruiter") == 25
...
def test_session_store_turn_counter() -> None:
    ...
    # 16th turn must be refused
    state = store.get(state.session_id)
    ok, used, limit = store.check_turn_quota("public", state)
    assert not ok
    assert used == 15 and limit == 15
